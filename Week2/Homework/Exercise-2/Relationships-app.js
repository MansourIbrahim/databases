...
// many to many relationship.
    ...
    async function exercise2() {
        ...
        try {
            ...
            const authorsPromises = AUTHORS_DATA.map(author => execQuery('INSERT INTO authors SET ?', author));
            const papersPromises = PAPER_DATA.map(paper => execQuery('INSERT INTO research_Papers SET ?', paper));
            const junctionPromises = JUNCTION_DATA.map(junction => execQuery('INSERT INTO authors_papers_junction SET ?', junction));
            await Promise.all(authorsPromises, papersPromises, junctionPromises);
            connection.end();
        } catch (err) {
